// max2alamo-2026 :: shaders/max-preview/BlobStencilMasked.fx
//
// Effects11 STUB shader for 3ds Max 2026 authoring preview.
// Mirrors the parameter surface of Petroglyph's stock BlobStencilMasked.fx for Max-side UI.
// This shader does NOT ship into EaW mods. See ../README.md.
//
// Auto-generated by scripts/generate-max-preview-stubs.py.
// Edit the manifest in that file and re-run to regenerate.


string _ALAMO_RENDER_PHASE = "Transparent";
string _ALAMO_VERTEX_PROC  = "Mesh";
string _ALAMO_VERTEX_TYPE  = "alD3dVertNU2";
bool   _ALAMO_TANGENT_SPACE = false;
bool   _ALAMO_SHADOW_VOLUME = false;


float4x4 WvpXf     : WorldViewProjection    < string UIWidget = "None"; >;
float4x4 WorldITXf : WorldInverseTranspose  < string UIWidget = "None"; >;

Texture2D <float4> Texture0 <
    string UIName = "Texture0";
    string UIType = "bitmap";
    string ResourceType = "2D";
>;
SamplerState Texture0Sampler {
    MinFilter = Linear; MagFilter = Linear; MipFilter = Linear;
    AddressU = Wrap; AddressV = Wrap;
};

Texture2D <float4> Texture1 <
    string UIName = "Texture1";
    string UIType = "bitmap";
    string ResourceType = "2D";
>;
SamplerState Texture1Sampler {
    MinFilter = Linear; MagFilter = Linear; MipFilter = Linear;
    AddressU = Wrap; AddressV = Wrap;
};

BlendState AlphaBlend {
    BlendEnable[0] = TRUE;
    SrcBlend = SRC_ALPHA;
    DestBlend = INV_SRC_ALPHA;
    BlendOp = ADD;
    SrcBlendAlpha = ONE;
    DestBlendAlpha = ZERO;
    BlendOpAlpha = ADD;
};
DepthStencilState NoZWrite {
    DepthEnable = TRUE;
    DepthWriteMask = ZERO;
};

struct VsIn {
    float4 Position : POSITION;
    float3 Normal   : NORMAL;
    float2 Tex      : TEXCOORD0;
};

struct VsOut {
    float4 HPosition : SV_Position;
    float3 WorldNrm  : TEXCOORD0;
    float2 UV        : TEXCOORD1;
};

VsOut std_VS(VsIn IN) {
    VsOut OUT = (VsOut)0;
    OUT.HPosition = mul(IN.Position, WvpXf);
    OUT.WorldNrm  = mul(IN.Normal, (float3x3)WorldITXf);
    OUT.UV        = IN.Tex;
    return OUT;
}

float4 std_PS(VsOut IN) : SV_Target {
    float4 base = Texture0.Sample(Texture0Sampler, IN.UV);
    float3 tint = float3(1,1,1);
    const float3 keyDir = normalize(float3(0.3, 0.6, 0.7));
    float ndl = saturate(dot(normalize(IN.WorldNrm), keyDir));
    float3 col = base.rgb * tint * (0.25 + 0.75 * ndl);
    return float4(col, base.a);
}

technique11 Main {
    pass P0 {
        SetBlendState(AlphaBlend, float4(0,0,0,0), 0xffffffff);
        SetDepthStencilState(NoZWrite, 0);
        SetVertexShader  (CompileShader(vs_5_0, std_VS()));
        SetGeometryShader(NULL);
        SetPixelShader   (CompileShader(ps_5_0, std_PS()));
    }
}
