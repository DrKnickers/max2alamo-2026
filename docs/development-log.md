...
| 11b | Modern multi-clip authoring + per-clip `.ala` emission. Walker change: `walk_animation` returns `vector<AlaAnimation>` (existing per-frame machinery iterates once per clip). Exporter change: per-clip filename emission `<basename>_<clipname>.ala`. Utility-panel backend: wire the existing Animation Settings rollout (UI was shipped in Phase 7-era Utility-UI work but backend disabled) to read/write `Alamo_Anim_Clips` + per-clip `_Start/_End` user props. No format-library changes (Phase 8a's `AlaAnimation::name` already round-trips per-clip name). Back-compat: un-suffixed `Alamo_Anim_Start/_End/_Name` continues to mean single-clip with the given name. Phase 8f visibility-propagation Utility button is the precedent for confident `alamo_utility.cpp` extensions. | pending | Architecture locked in Phase 11a. 3 new harness tests planned (`test_phase11_multi_clip`, `test_phase11_legacy_single_clip_back_compat`, `test_phase11_no_clips_no_ala`) + 12 prior gold SHAs preserved as hard merge gate + 3 tripwires. Estimate 1–2 sessions. |
| 11c | Legacy `.max` clip-data reader. Register a hidden `ClassDesc` with `Class_ID(0x70a24090, 0x60c90f03)`, `SuperClassID = 0x1020`, `IsPublic = FALSE` (mirrors Phase 10d's hidden-ClassDesc pattern for the legacy proxy class). Implement `UtilityObj::Load(ILoad*)` to decode the per-clip 342-byte appData stream into `(name, start, end)` records, then translate to Phase 11b's `Alamo_Anim_Clips` + `Alamo_Anim_<NAME>_Start/_End` user-prop convention so the Animation Settings rollout populates automatically when a legacy file opens. Phase 11a unblocked this by recovering the Class_ID + record stride + name offset. Fallback: if Max doesn't route legacy `appData` to our registered ClassDesc (because legacy plugin used non-standard SCID 0x1020 in a way that breaks registration), drop to Strategy 1 — scan `.max` bytes for Class_ID, walk 342-byte stride, decode names from offset +34 — which is a ~50-line prototype against EI_SNOWTROOPER.max. | pending | Class_ID resolved; record stride + name offset decoded; start/end frame offsets within record TBD as part of 11c implementation. Test fixture: `EI_SNOWTROOPER.max` (60 clip records) + `CIS_SBD.max` (8 clip records) + `Stormtrooper.max` (60). Estimate 1–2 sessions. |
