...
| 13b | Phase 10.5 (issue [#81](https://github.com/DrKnickers/max2alamo-2026/issues/81)) — `0x10006` skin-bone-remap writer + Phase 11c-followup legacy clip importer made always-active. **Both fixes ship together** ([PR #83](https://github.com/DrKnickers/max2alamo-2026/pull/83) at `63f5654`) — both surfaced by the EI_SNOWTROOPER Tier 4 of PR #80. New module [`alamo_format::skin_bone_remap`](../alamo_format/include/alamo_format/skin_bone_remap.h) builds a per-submesh local-to-global bone remap in first-seen order + rewrites each vertex's `bone_indices[0..3]` from globals to local slots; writer emits the `0x10006` chunk inside `0x10000` between `0x10004` and `0x1200`; walker calls `apply_skin_bone_remap` for any submesh whose `vertex_format_name` is `RSkin*` / `B4I4*`. 9 new Catch2 cases (177/177 total, was 168). Empirical Tier 4: user re-exported `EI_SNOWTROOPER.alo` and confirmed Snowtrooper renders as humanoid in AloViewer — no more stretched-triangle artifacts. Closes #75's R8 question. **11c-followup**: legacy clip importer was registered inside `AlamoUtility::BeginEditParams` (only fired when user had clicked Utilities → Alamo Utility BEFORE opening a legacy `.max`) AND compounded by `CanAutoDefer=1` deferring DLL load until first use. Fix: `CanAutoDefer=0`; new `LibInitialize` / `LibShutdown` exports register a process-lifetime `NOTIFY_FILE_POST_OPEN` hook via `RegisterLegacyClipImportNotifications()` in [`legacy_clip_importer.cpp`](../max2alamo/src/legacy_clip_importer.cpp). Tier 4 confirmed: opening `EI_SNOWTROOPER.max` now triggers `"max2alamo: imported 60 legacy animation clip(s)"` in the Listener and export emits 60 `<basename>_<CLIP>.ala` siblings. **Animation playback still broken** — surfaced during same Tier 4 but is a separate **pre-existing Phase 8 issue** (single-bone synthetic test scenes hid it). Tracked as [#84](https://github.com/DrKnickers/max2alamo-2026/issues/84) with hypotheses ruled out (translation-delta, quat convention, MAX_NUM_SKIN_BONES limit) and a focused diagnostic playbook for the next session. | ✅ shipped | Catch2 tests passing 177/177 (8 new TEST_CASEs in `skin_bone_remap_test.cpp` covering rigid-attachment / multi-bone-weighted / empty-submesh / weight-preservation / idempotence / determinism / both format-family-detection cases; 1 new TEST_CASE in `alo_build_test.cpp` with 3 SECTIONs pinning `0x10006` flow-through). Walker / writer changes don't touch round-trip codepath; existing 4929/4929 corpus pass preserved. .dle rebuilt cleanly; install hook drops it at `plugin/max2alamo.dle`. |
| 14 | Animation playback fixes + static-mesh hierarchy ([#84](https://github.com/DrKnickers/max2alamo-2026/issues/84)) — three sub-phases in [PR #86](https://github.com/DrKnickers/max2alamo-2026/pull/86). **14a (the headline fix)**: Max SDK's `Quat(Matrix3)` constructor returns the conjugate of what the `.ala` format expects; one-line fix in [`scene_walker.cpp::extract_rotation_quat`](../max2alamo/src/scene_walker.cpp#L1582) negates xyz before packing. The `.alo` 0x206 path doesn't go through quat extraction so rest poses were correct — the bug was only on the per-frame `.ala` quats and compounded through 36-bone rigs to manifest as "rotated 90 degrees forward, but not properly" on EI_SNOWTROOPER. Diagnostic path: new `tools/ala_diff/` typed-semantic `.ala` comparator + side-by-side against vanilla `EI_TROOPER_IDLE_00.ALA` showed B_Pelvis quat `(-0.478, +0.523, -0.454, +0.540)` vs vanilla `(+0.478, -0.523, +0.454, +0.540)` — textbook conjugate signature. **14b (gun follows hand, data-layer)**: `walk_node`'s `!is_skinned` branch now inherits the Max-side parent bone instead of hardcoding scene Root; the synth attachment bone is encoded in parent-bone-local space via `mesh_world × Inverse(parent_world)`. Legacy fallback (no Max parent / parent not in `bone_map`) preserves the prior child-of-Root behavior — `test_static_*` / `test_proxy_*` / `test_mesh_*` suites unchanged. EI_SNOWTROOPER's `Box01` now parents to `B_Gun` chaining up through `B_Hand_R → B_FArm_R → … → Root`. **14c (investigation, no fix)**: visually the gun renders at the wrong world position despite 14b's data-layer correctness. Built `tests/maxscript/verify/_chain_dump.py` (computes engine's apparent reconstructed world via row-vector chain composition) and ran side-by-side against an in-Max MAXScript dump. The sharpest single clue is from the 14b synthetic fixture: HandBone authored at world `(500, 0, 0)` with `+90°` Z rotation comes out as encoded translation `(0, -500, 0)`, exactly `world × Inverse(R_+90°_Z)`. Pointed at one of: stray `Inverse(R)` call in walker / column-vector composition convention / `IGameNode::GetLocalTM` SDK quirk. Body bones also drift from Max's view by ~3-5 units but render correctly visually because offsets are internally consistent; the gun's `~11`-unit X offset is much larger. **Phase 14d** (tracked separately) will discriminate between the candidate explanations via fixture-rotation variation + AloViewer source read. | ✅ shipped (14a + 14b; 14c investigation; known issue: 14b's data-layer fix produces visible gun misalignment on bone-parented static meshes until 14d) | (14a) Post-fix EI_SNOWTROOPER IDLE_00 B_Pelvis quat matches vanilla EI_TROOPER's byte-for-byte; 15/18 shared bones byte-match within epsilon; visual confirmation in AloViewer through all 60 clips. (14b) `tests/maxscript/test_phase14b_static_mesh_parent_bone.ms` + its verifier assert `GunMesh.parent_index = 1` (HandBone) and translation `(200, 0, 0)` in parent-local space. EI_SNOWTROOPER's gun mesh's synth attachment bone (`Box01`) now correctly chains up through the right hand. Full static-prop/proxy/mesh suite (`test_static_*` 1/1, `test_proxy_*` 4/4, `test_mesh_*` 1/1) preserved post-fix. (14c) `_chain_dump.py` + `_phase14c_dump.ms` (untracked) produced side-by-side Max-vs-engine data with concrete deltas (body bones ~3-5 units; gun ~11 units on X, ~6 on Z) and the `(0, -500, 0)` synthetic-fixture clue. Findings doc at [`docs/plans/phase-14c-gun-misaligned-findings.md`](plans/phase-14c-gun-misaligned-findings.md). |
| 14d | Gun-misalignment investigation closed without a walker change ([PR #88](https://github.com/DrKnickers/max2alamo-2026/pull/88), commit `5cea720`). Phase 14c's "(0, -500, 0)" clue turned out to be **two distinct content-authoring issues** stacked on the same symptom, not a walker convention bug. **Cause A (synthetic fixture)**: `BoneSys.createBone [500,0,0] [500,1000,0]` (direction +Y) bakes `+90° Z` into HandBone's node TM; the `at time 0 (hand.rotation = identity)` keyframe then orbits the translation around the local pivot to `(0, -500, 0)` — fixture's own bind pose silently corrupted. Walker faithfully encoded that. Fixed by switching createBone direction to collinear-X so node TM stays identity. **Cause B (EI_SNOWTROOPER)**: `sliderTime = 373f`, `animationRange = (interval 373f 2488f)` — frame 0 is OUTSIDE the animation range, so `Box01.position`'s `Position_XYZ` controller falls back to leftover initial values `(-4.49, -1.06, 0)` at frame 0 (the floor). Walker correctly samples that frame-0 position because `igame->SetStaticFrame(0)` ([`scene_walker.cpp:1966`](../max2alamo/src/scene_walker.cpp#L1966)) makes the bind pose = frame 0 always. Documented as authoring requirement in [`docs/build.md`](build.md) "Authoring convention — bind pose lives at frame 0". Four diagnostic variants ([`tests/maxscript/_phase14d_variants.ms`](../tests/maxscript/_phase14d_variants.ms)) prove walker encoded translation = Max's `node.transform.row4` in every case (identity, asymmetric rotation, both). | ✅ shipped (no walker change; fixture corrected + authoring convention documented) | All four diagnostic variants encoded-T = Max-T match. Same-time chain-dump of EI_SNOWTROOPER (`_phase14d_snowtrooper_dump.ms`) confirms walker output = Max's frame-0 view exactly for B_FArm_R, B_Hand_R, B_Gun, Box01. Investigation writeup at [`docs/plans/phase-14d-no-walker-bug-findings.md`](plans/phase-14d-no-walker-bug-findings.md). Phase 14b walker change confirmed correct as-is. |
| 14e | Static-mesh accessories drift during animation playback ([PR #89](https://github.com/DrKnickers/max2alamo-2026/pull/89), commit `b51c93b`, closes [#87](https://github.com/DrKnickers/max2alamo-2026/issues/87)). User reported: after 14d landed, bind pose render correct, but during animation playback the rifle snapped ~7.7 units off the hand. Diagnostic via [`tests/maxscript/verify/_ala_dump_bone.py`](../tests/maxscript/verify/_ala_dump_bone.py) (new) showed Box01's `.ala` track had `idx_t=-1, idx_r=-1, trans_offset=(0,0,0), default_rotation=(0,0,0,32767)`. Compared to vanilla `EI_DARKTROOPER_ONE_ATTACK_00.ALA`: every non-animatable bone there (MuzzleA_00, Camera01.Target, IK_*, etc.) carries `trans_offset` = bind translation and `default_rotation` = bind rotation as packed quat. Walker `walk_animation` ([`scene_walker.cpp`](../max2alamo/src/scene_walker.cpp)) was setting these only for bones in `bone_map` (animatable real bones), leaving synth bones at struct zeros. Engine reads "Box01 is at parent-local (0,0,0) with identity rotation" the moment any clip starts playing → gun snaps to B_Gun's origin. **Fix**: parallel pass after the animatable-bone `default_rotation` loop that decodes `scene.bones[i].matrix` back into a Matrix3, copies translation row to `trans_offset`, extracts rotation Quat via existing `extract_rotation_quat` / `pack_quat_int16` helpers into `default_rotation`. ~38 LoC. | ✅ shipped | User-confirmed: re-exported EI_SNOWTROOPER plays clips with rifle attached to hand throughout. Box01 `.ala` post-14e: `trans_offset=(-0.076, -0.840, -7.689)` matching bind matrix, `default_rotation=(9,-9,20,32767)` near-identity matching bind's micro-noise. 6 prior-passing animation tests (test_phase8_anim_*, test_phase11_*, test_phase14b_*) still pass. 3 pre-existing fails (phase8_acceptance, rotation_keyframes, translation_keyframes) trace to Phase 14a quat-conjugate change, not 14e — closed by PR #90's verifier regen. Phase 14d's docs-only conclusion was right for what it diagnosed (fixture orbit + degenerate frame-0) but wrong about there being "no walker bug" — 14e closes that gap. |
| 14a-regen | Three verifier expectations broken by Phase 14a's `Quat(Matrix3)` conjugate ([PR #90](https://github.com/DrKnickers/max2alamo-2026/pull/90), commit `d4200d6`). `verify_test_rotation_keyframes.py` #14, `verify_test_translation_keyframes.py` #C17, `verify_test_phase8_acceptance.py` #D23/D25/D27/D28 all failed with `delta = √2` (pure `q` vs `-q`) since 14a shipped — chronic 3-test FAIL on every `scripts/run-max-tests.ps1` run. SESSION_NOTES had carried this regen as pending. Most fixes are mechanical xyz-sign flips in the expected literals + comments citing `bf35d5e` for the new convention. **#D28 needed structural change**: walker post-14a emits `conj(q_offset_true * q_authored)`, and because `conj(a*b) = conj(b)*conj(a)`, multiplying on the LEFT by `q_offset_inv` no longer collapses the offset. Switched to RIGHT-multiply (`qs[30] * q_offset_inv`) so the `conj(q_offset_true) * q_offset_true` tail collapses to identity, leaving `conj(q_authored)`. Same geometric invariant (Phase 5g composition stays intact through 8b's per-frame quat path), different math expressing it. | ✅ shipped | All 3 previously-failing verifiers now pass against the existing `build/maxbatch/*.alo` + `*.ala` files. No walker / format-library change; verifier-side only. |
| 10.5-regen | `verify_test_skinned_rskin.py` broken by Phase 10.5's `0x10006` skin-bone-remap ([PR #91](https://github.com/DrKnickers/max2alamo-2026/pull/91), commit `2a0814b`). Pre-Phase-10.5 the writer emitted `BoneIndices` as global skeleton indices; post-Phase-10.5, for shaders that need it, `BoneIndices` are LOCAL slot indices and the engine dereferences via a per-submesh `0x10006` chunk. Verifier had been asserting `bone_indices[0] in {1,2,3}` (global), saw `{0, 2, 3}` (local), and failed — even though `0x10006 = (1, 0, 2, 3)` correctly resolves the locals to globals `{1, 2, 3}`. Surfaced during v0.9.1 release prep when the harness ran 58/59 OK + 1 FAIL after PR #90's regen cleared the earlier abort point. **Fix**: `_alo.py` now parses `0x10006` into `Submesh.bone_remap` and exposes `Submesh.resolve_bone(local_idx)` helper (additive change; default `None` makes it a no-op for non-remap submeshes). Verifier calls `sm.resolve_bone(v.bone_indices[0])` before asserting in `{1, 2, 3}`. | ✅ shipped (last gap closed for v0.9.1 release-readiness; harness now 59/59 clean) | All 9 skinning-touching verifiers (test_skinned_cylinder, test_biped_skinned, test_phase8_skinning_stress, test_phase8_biped_animated, test_smooth_skinned_joint, test_pivot_skinned_safety, test_phase7_acceptance, test_phase8_acceptance, test_two_meshes_offset) still PASS post-fix because `_alo.py`'s change is additive. Corpus round-trip pre-v0.9.1 check: 4,929 / 4,929 PASS = exact match to v0.9.0 baseline (writer changes are all in `scene_walker.cpp`, not in `alamo_format/`). |
...
| v0.9.1 release | First stabilization-window release on top of v0.9.0's feature-complete shot, **published 2026-05-17 14:09 UTC** at [github.com/.../releases/tag/v0.9.1](https://github.com/DrKnickers/max2alamo-2026/releases/tag/v0.9.1). Six engine-visible writer correctness fixes since v0.9.0 (vertex-format-string per shader, `0x10006` skin-bone-remap, animation rotation-quat sign, static-mesh parent inheritance, static-mesh `.ala` defaults, legacy clip-data importer always-active) plus the test-side regen and diagnostic tooling that surfaced them — see Phase 14d / 14e / 14a-regen / 10.5-regen rows above and [`docs/release-notes/v0.9.1.md`](release-notes/v0.9.1.md) for the per-fix breakdown. Built from tag commit `ac3125c` ([PR #92](https://github.com/DrKnickers/max2alamo-2026/pull/92)) via the standard [`docs/release.md`](release.md) ceremony: CI release workflow created the draft with 3 CLI/lib assets (3m20s build); `.dle` built locally from a detached-HEAD worktree at the tag (Risk #1 mitigation per the release plan), attached via `gh release upload`, published via `gh release edit --draft=false`. | ✅ released | Asset SHA-256s (recorded for reproducibility): `max2alamo.dle` 306,688 B = `5e73a4b1bea5a1ee8becedb63af9e4f9a959a6ce3ac2ea9d9388eb2ac6195de6`; `alamo_format.lib` 958,074 B = `377711f662a84101c9c66c2413ec3a3d855dc1e39ccdaa25c1be80b3be21a53b`; `alo_dump.exe` 51,200 B = `59e29fbcebf5bbb59a7dc7df209188fe135c6a386eec923db0da339f13b58854`; `alo_roundtrip.exe` 75,776 B = `7321802b339f44e00b881aac0dc77508cf6a35e10f58d4e5df3f38e56f4c9389`. Pre-tag verification (against the merge commit `ac3125c`): Catch2 suite passes; corpus byte-identity 4,929 / 4,929 (1,383 + 683 `.alo` + 1,500 + 1,363 `.ala`) — exact match to v0.9.0 baseline; Max-side harness `scripts/run-max-tests.ps1` runs 59 / 59 clean (modulo the unrelated `test_phase14_min_chain` missing-verifier note that pre-dates this release). |
...
### Phase 14 — Animation playback fixes + static-mesh hierarchy (shipped with known issue)

Three sub-phases shipping together in [PR #86](https://github.com/DrKnickers/max2alamo-2026/pull/86), all sub-issues of [#84](https://github.com/DrKnickers/max2alamo-2026/issues/84) — the "animation playback wrong on EI_SNOWTROOPER" bug surfaced during Phase 13b's Tier 4. Plan + findings docs live at [`docs/plans/phase-14-diagnostic.md`](plans/phase-14-diagnostic.md), [`docs/plans/phase-14b-static-mesh-bone-parent.md`](plans/phase-14b-static-mesh-bone-parent.md), [`docs/plans/phase-14c-gun-misaligned-findings.md`](plans/phase-14c-gun-misaligned-findings.md).

#### Phase 14a — Quaternion-conjugate fix

Root cause: Max SDK's `Quat(Matrix3)` constructor returns the conjugate of what the `.ala` format expects. The `.alo`'s `0x206` 4x3 bone matrix doesn't go through quat extraction — only the per-frame `.ala` quats do — so the bug was invisible in the static bind pose and surfaced only during animation. Compounded through 36 bones on EI_SNOWTROOPER, the inverse rotation per frame manifested as "rotated 90 degrees forward, but not properly."

Fix in [`scene_walker.cpp:1582`](../max2alamo/src/scene_walker.cpp#L1582) `extract_rotation_quat`: conjugate the result before returning. `Quat(-q.x, -q.y, -q.z, q.w)`. One-line change. `.alo` rest poses unaffected (the 0x206 matrix encoding path doesn't call this function).

Diagnostic path that found it: built `tools/ala_diff/` (typed semantic comparator for `.ala` files; reads via the existing `alamo_format::read_ala` and compares header / per-bone metadata / per-frame unpacked tracks with epsilon), exported EI_SNOWTROOPER through `3dsmaxbatch`, side-by-side compared `IDLE_00.ala` against vanilla `EI_TROOPER_IDLE_00.ALA` from the corpus. B_Pelvis showed:
...
Same magnitudes, x/y/z signs flipped, w unchanged — the textbook quaternion conjugate signature. Post-fix B_Pelvis byte-matches vanilla; 15 of 18 shared bones in Snowtrooper IDLE_00 byte-match vanilla EI_TROOPER IDLE_00 within epsilon (the 3 outliers are asset-authoring differences between the ThrREv mod's Snowtrooper rig and stock vanilla — not walker bugs).

Visual confirmation in AloViewer: post-fix, the Snowtrooper animates correctly through all 60 clips with the body moving as expected.

#### Phase 14b — Static-mesh inheritance from Max-parent bone

After 14a landed, EI_SNOWTROOPER's rifle still floated free of the hand during playback (data-layer correctness ≠ visual rendering — surfaced by the user's AloViewer screenshot). Root cause: our walker hardcoded `parent_index = 0` for all static (non-skinned) meshes' per-mesh synthetic attachment bones, regardless of whether the mesh's Max-side parent was an exportable bone.

Fix in [`scene_walker.cpp:1267`](../max2alamo/src/scene_walker.cpp#L1267) `walk_node`'s `!is_skinned` branch: look up `max_node->GetParentNode()` in `bone_map` (populated upstream by `walk_bones`). If found, set `synth_bone.parent_index` to that bone's index and encode the matrix as `mesh_world × Inverse(parent_world)` (parent-local TM). Otherwise fall through to the legacy "child of Root + world TM" path — existing static-prop, proxy, and mesh-hierarchy suites preserved.

Tests: `tests/maxscript/test_phase14b_static_mesh_parent_bone.ms` builds a Box parented to a BoneSys HandBone with HandBone keyframed; the verifier asserts `GunMesh.parent_index = 1` (HandBone, not Root) and translation `(200, 0, 0)` in HandBone-local space. Red-then-green proved: pre-fix the verifier exits 1 with both regression markers naming `parent_index = 0` and translation `(200, -500, 0)`; post-fix exits 0 with translation `(200.0, 3e-5, 0.0)`.

Data-layer verification on EI_SNOWTROOPER: post-fix, the gun mesh's attachment bone `Box01` parents to `B_Gun` (bone index 15), chaining up through `B_Hand_R → B_FArm_R → B_Bicep_R → B_Chest → Turret_00 → B_Pelvis → Root`. The muzzle-flash attachment bone similarly parents to `MuzzleA_00`. The collision-hull bone `Col_Main` legitimately stays at Root (correct — whole-figure proxy, not attached to any body part).

**Known issue (Phase 14c-tracked):** despite the data-layer correctness, the gun visibly renders at the wrong world position in AloViewer — far-left of the body, near the floor. Workaround until Phase 14d resolves: author static accessories WITHOUT parenting them to a bone in Max — they attach to scene Root and stay at their bind-pose world position (pre-Phase-14b behavior; they won't follow animation).

#### Phase 14c — Gun-misalignment investigation (no fix)

Investigation-only follow-up; built two diagnostic tools to characterise the 14b visual misalignment:

- [`tests/maxscript/verify/_chain_dump.py`](../tests/maxscript/verify/_chain_dump.py) — reads a `.alo` and chain-composes the bone hierarchy via row-vector convention to compute the engine's *apparent* reconstructed world TM per bone. Documents the on-disk format layout from `encode_matrix3` and Mike Lankamp's reader.
- `tests/maxscript/_phase14c_dump.ms` (untracked, hardcoded user-specific path) — loads `EI_SNOWTROOPER.max` and dumps Max's view of Box01, B_Gun, B_Hand_R world TMs.

Side-by-side data, with deltas:

| Node | Max world | Row-vector chain-composed | Delta (Max - Engine) |
|---|---|---|---|
...
**Sharpest clue** (the synthetic 14b fixture, single top-level bone, identity object offset): HandBone authored at Max world `(500, 0, 0)` with rotation `+90°` about Z comes out as encoded `.alo` translation `(0, -500, 0)` — *exactly* `world × Inverse(R_+90°_about_Z)`. That points at one of: a stray `Inverse(R)` call in the walker, a column-vector composition convention (equivalent to applying `Inverse(R)` on top of row-vector results for orthogonal rotations), or an `IGameNode::GetLocalTM` SDK quirk that returns world-with-inverse-rotated-translation rather than plain world.

Body bones also disagree with Max's view (~3-5 unit deltas) but render correctly visually because all bones are encoded consistently — Snowtrooper composes into a recognizable upright humanoid even though the absolute world coordinates don't match Max. The gun's ~11-unit X delta is much larger because the Phase 14b fix computed `mesh_world × Inverse(parent_world)` in Max's coord-space, then encoded it for an engine that composes through a different convention — making the gun's transform inconsistent with the rest of the chain.

None of the four original H-A/B/C/D hypotheses from the investigation plan fit cleanly. The recommended Phase 14d sequence (per the final reviewer): (1) vary the 14b fixture's HandBone rotation through identity, `+45°` about X, `+90°` about Y; dump the resulting `.alo` translation rows; the pattern of which inverse rotation appears discriminates the candidate explanations cheaply. (2) Then read AloViewer's [`src/Assets/Models.cpp`](https://github.com/GlyphXTools/alo-viewer/blob/master/src/Assets/Models.cpp) bone-chain-compose code to confirm the convention. (3) Empirical "load in AloViewer" cross-check is third priority.

#### Out of scope / follow-up

- **Phase 8 SHA test regeneration**: 14a's quat-conjugate change broke gold SHAs in `tests/maxscript/test_phase8_*` and any rotation-keyframe test that fixed-string-compares against `.ala` bytes. Need to regenerate; tracked separately.
- **AloViewer-playback verification gate**: the Tier 4 coverage hole that allowed both 14a and 14b to slip through Phase 8's byte-only checks. A side-by-side `ala_diff` dump-vs-vanilla check at fix time would have caught the convention bugs immediately. The `ala_diff` tool now exists; the gate is the missing harness piece.
- **Phase 14d**: composition-convention resolution per the 14c plan; will produce the actual visual fix for static accessories on bones.
