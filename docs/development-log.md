...
| 11b.2 | Utility-panel backend wiring for Phase 11b. Connect the existing Animation Settings rollout in `max2alamo/resources/utility_dialogs.rc` (UI shipped in the Phase 7-era Utility-UI work but backend disabled per the Utility-UI dev-log entry) to the Phase 11b.1 user-prop convention: combo box populates from `Alamo_Anim_Clips`; Start/End spinners read/write `Alamo_Anim_<NAME>_Start/_End` on rootNode; Add/Del buttons mutate the list (with `[A-Z0-9_]` validation on names); `<<` / `>>` navigate the list; `Display Current` scrubs Max's `animationRange` to the selected clip; `Display All` scrubs to the span of all clips. All writes wrapped in `undo on` for clean ctrl-Z, matching the Phase 8f visibility-propagation button precedent. Includes manual Max-GUI smoke test (not 3dsmaxbatch-amenable — the harness can't fire mouse clicks). | pending | Estimate 1 session. CLI multi-clip authoring already works via the Property Editor; backend wiring is QoL for the Utility panel. |
