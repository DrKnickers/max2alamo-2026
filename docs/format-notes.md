...
| `_ALAMO_VERTEX_TYPE` | override vertex format selection (must be one of the 15 names in AloViewer's `VertexFormatNames` table; see Q6 below + [#75](https://github.com/DrKnickers/max2alamo-2026/issues/75)) |
...
| 4 | ~~Format-version indicator: how does the engine distinguish EaW-style vs FoC-style `.ala` for a given file?~~ **Resolved Phase 8:** presence/absence of mini-chunks 11/12/13 in `0x1001` is the signal (per `alamo2max.ms:855-861`). |
...
| 6 | ~~What does `_ALAMO_VERTEX_TYPE` accept as values?~~ **Resolved (investigation, 2026-05-15):** the engine's renderer maps the `0x10002` string via case-insensitive lookup into AloViewer's 15-entry [`VertexFormatNames`](https://github.com/GlyphXTools/alo-viewer/blob/master/src/RenderEngine/DirectX9/VertexFormats.cpp) table — `alD3dVertN`, `alD3dVertNU2`, `alD3dVertNU2C`, `alD3dVertNU2U3U3`, `alD3dVertNU2U3U3C`, `alD3dVertRSkinNU2{,C,U3U3,U3U3C}`, `alD3dVertB4I4NU2{,C,U3U3,U3U3C}`, `alD3dVertGrass`, `alD3dVertBillboard`. The string IS load-bearing: it selects the GPU vertex declaration (which controls bone-influence count — RSkin reads 1 bone via packed `Normal.w`, B4I4 reads 4 bones with `BlendWeights[4]`+`BlendIndices`). The vertex chunk ID (`0x10005` legacy vs `0x10007` rev 2) only informs the loader's byte-decode path; AloViewer normalizes both to a 144-byte `MASTER_VERTEX` post-load. Our walker hardcodes `alD3dVertNU2` regardless of shader at `alamo_format/src/alo_build.cpp:18` — bug tracked as Phase 10 in [#75](https://github.com/DrKnickers/max2alamo-2026/issues/75). Corpus survey via `scripts/survey_vertex_formats.py` (10,737 vanilla submeshes, 60 unique (shader, vfmt, chunk-id) triplets) confirms strict 1-to-1 shader↔string mapping in vanilla, with 10 of the 15 names in use. Vertex chunk ID split: `0x10007` 98.6%, `0x10005` 1.4%. |
| 6b | `0x10006` chunk classification. **Resolved (investigation, 2026-05-15):** per AloViewer [`src/Assets/Models.cpp:155`](https://github.com/GlyphXTools/alo-viewer/blob/master/src/Assets/Models.cpp), `0x10006` is **NOT a vertex-data chunk** — it's a per-submesh skin-bone remap table: a flat list of `uint32` global bone indices that re-maps the per-vertex `BoneIndices[0..3]` from local-to-submesh space to global-to-skeleton space. Optional in AloViewer's loader. Corpus distribution: 779 / 10,737 vanilla submeshes carry one, ALL on `RSkin*` (703) or `B4I4*` (76) skinned shaders, none on static-mesh shaders. Our walker doesn't emit it — open question whether the shipping engine requires it for correct skinned rendering. Listed as Risk 8 in #75; resolution path documented in `docs/plans/phase-10-prereq.md`. |
