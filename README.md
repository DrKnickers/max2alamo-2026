...
- **[Mike Lankamp (Mike.NL)](https://mike.nl/)** — this plugin stands on his work. His reverse-engineering of the `.alo`/`.ala` formats, AloViewer, and the `alamo2max` MAXScript importer are the foundation `alamo_format` is built on, and AloViewer remains the ground-truth visual check for every export.
- **[Gaukler](https://github.com/Gaukler)** — the [Blender ALAMO plugin](https://github.com/Gaukler/Blender-ALAMO-Plugin), an invaluable second working reference for material parameters, vertex formats, and the bump-mapping shader list.
- **[Petroglyph](https://www.petroglyphgames.com/)** — for shipping EaW/FoC as a moddable game and releasing the shader source. Twenty years on, the modding community they supported is still writing new tools for it.
